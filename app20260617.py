...
# =========================================================
# 1. 核心计算逻辑：沿用 corecode.py 中的计算公式，仅保留界面需要的三类方法
# =========================================================
...
OVERRIDE_KEYS = ["SH", "SE", "Ssigma", "Sbs", "Sy", "SA", "SI", "SK", "Sd", "SF"]
...
def _validate_section(section_type: int, params: Dict[str, float], prefix: str) -> None:
    if section_type == 1:
        ...
        if 2 * params["t"] >= params["D"]:
            raise ValidationError(f"{prefix}圆环截面参数错误：2t 必须小于 D")
        ...
        if 2 * params["tw"] >= params["B"]:
            raise ValidationError(f"{prefix}空心方钢参数错误：2tw 必须小于 B")
        ...
        if 2 * params["tf"] >= params["h"]:
            raise ValidationError(f"{prefix}工字钢参数错误：2tf 必须小于 h")
        if params["tw"] >= params["d"]:
            raise ValidationError(f"{prefix}工字钢参数错误：tw 必须小于 d")

    else:
        raise ValidationError("截面形式输入错误")
...
def calculate_similarity(
    ...
    if method_type not in METHOD_NAMES:
        raise ValidationError("应用场景输入错误")

    _check_positive("几何基准相似比 SCB", scb)
    _check_positive("原型构件 E", proto.get("E"))
    _check_positive("原型构件 fy", proto.get("fy"))
    _check_positive("原型构件 bs", proto.get("bs"))
    _check_positive("原型构件 l", proto.get("l"))
    _check_positive("缩尺模型 E", model.get("E"))
    _check_positive("缩尺模型 fy", model.get("fy"))
    _check_positive("缩尺模型 bs", model.get("bs"))
    _check_positive("缩尺模型 l", model.get("l"))

    proto_section = proto.get("section", {})
    model_section = model.get("section", {})

    _validate_section(section_type, proto_section, "原型构件")
    _validate_section(section_type, model_section, "缩尺模型")

    section_name = SECTION_NAMES[section_type]
    method_name = METHOD_NAMES[method_type]

    Sy = _calc_sy(section_type, proto_section, model_section)
    ...
    if user_inputs.get("Ssigma") is not None:
        Ssigma_real = user_inputs["Ssigma"]
...
def reset_input_state() -> None:
    ...
    keys_to_delete += [
        "section_select", "method_select", "scb_input", "result",
        "input_snapshot", "override_snapshot", "tables_snapshot",
        "calculation_result", "method_input_snapshot",
    ]
...
def make_result_tables(result: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    derived_rows = [
        ...
        ["SK", "等效刚度相似比", result.get("SK"), "由 E、I、l 的相似关系确定"],
...
def render_result_cards(result: Dict[str, Any]) -> None:
    ...
    cards = [
        ...
        ("恢复力相似比 SF", result.get("SF"), "关键参数"),
    ]
    if result.get("methodType") == 4:
        cards.append(("应力相似比 Ssigma", result.get("Ssigma"), "关键参数"))

    columns = st.columns(len(cards), gap="small")
    for col, (title, value, foot) in zip(columns, cards):
        with col:
    ...
    with st.sidebar:
        ...
        with col1:
            if st.button("清空结果", use_container_width=True):
                for key in ["result", "input_snapshot", "override_snapshot", "tables_snapshot"]:
                    st.session_state.pop(key, None)
                st.rerun()
        with col2:
        ...
        st.markdown("---")
        st.markdown("### 使用提示")
        st.caption("建议先完成参数输入，再选择应用场景；若已有部分相似比，可在高级设置中手动指定。")
    ...
    st.markdown(
        ...
        <div class="hero-card">
            ...
            <div style="opacity:0.94; line-height:1.6; max-width:1080px;">
                ...
                内置五类常用截面形式下的基本参数、衍生参数与关键参数相似比自动求解。支持用户上传自定义截面形式，将自动识别以完成求解。
            </div>
            <div class="hero-tags">
...
def render_input_tab(api: SimilarityAPI) -> None:
    ...
    top_col1, top_col2, top_col3 = st.columns([1.05, 1.35, 1.0], gap="large")
    with top_col1:
    ...
    if section_record.source == "builtin":
        ...
        st.markdown('<div class="section-caption">截面参数</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="section-note">根据当前用户扩展截面配置输入几何参数。</div>',
            unsafe_allow_html=True,
        )
    ...
    with hint_col:
        st.markdown(
            ...
            计算前系统将自动检查正数约束与截面几何约束。若参数不合理，将给出明确提示。
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if do_calculate:
        try:
            proto, model = build_param_dict(proto_mat, model_mat, proto_section, model_section)
            ...
            st.session_state["input_snapshot"] = {"proto": proto, "model": model}
    ...
    if extra_outputs:
        with st.expander("查看扩展组件附加结果", expanded=False):
            extra_df = pd.DataFrame(
                [{"参数": key, "数值": format_num(value)} for key, value in extra_outputs.items()]
            )
...
def render_help_tab() -> None:
    ...
    with col1:
        st.markdown(
            ...
            本工具用于缩尺拟静力试验中的非完全相似比确定。用户输入原型构件与缩尺模型的材料、构件长度和截面参数后，
            系统自动计算基本参数、衍生参数和关键参数相似比。
            </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            """
            <div class="small-card">
            <div class="section-caption">支持截面</div>
            <div class="section-note">
            圆形、圆环、方形、空心方钢、工字钢。不同截面将自动调用对应面积与惯性矩计算公式。
            </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with col2:
        st.markdown(
            ...
            <div class="section-note">
            DA 用作对照计算；CSL-Y 面向已完成拟静力试验的结果换算；CSL-N 面向拟进行拟静力试验的相似比设计。
            </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            ...
            <div class="section-note">
            系统会检查所有输入是否为正数，并检查圆环、空心方钢、工字钢的基本几何约束，避免无效参数进入计算。
            </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    st.markdown(
        ...
        <ol>
            <li>选择截面形式。</li>
            <li>输入原型构件与缩尺模型的材料参数、构件长度和截面参数。</li>
            <li>选择拟静力试验应用场景。</li>
            <li>如已有部分相似比，在高级设置中勾选并输入指定值。</li>
            <li>点击“开始计算”，查看结果摘要和分类结果表。</li>
